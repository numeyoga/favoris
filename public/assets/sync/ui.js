...
function openModal(sync) {
  ...
  function renderConfigForm(s, body, rerender) {
    ...
    s.adapters.forEach(a => {
      const o = el('option', null, a.label); o.value = a.id;
      if (a.id === pickedProvider) o.selected = true;
      sel.appendChild(o);
    });
    sel.onchange = () => { pickedProvider = sel.value; rerender(); };
    ...
    const cancel = el('button', 'btn', 'Annuler'); cancel.onclick = close;
    ...
    ok.onclick = async () => {
      ...
      if (missing) { toast('Tous les champs sont requis'); return; }
      ok.disabled = true; ok.textContent = 'Connexion…';
      try {
  ...
  function renderSignIn(s, body, rerender, close, toast) {
    body.appendChild(el('p', null,
      ...
      s.provider + '</strong>. Connectez-vous par lien magique pour activer la synchronisation.</span>'));

    const field = el('div', 'field');
    field.appendChild(el('label', null, 'Email'));
    const ip = el('input'); ip.type = 'email'; ip.placeholder = 'vous@exemple.fr';
    field.appendChild(ip);
    body.appendChild(field);
    ...
    reconf.onclick = async () => { await sync.forget(); render(); };
    const send = el('button', 'btn primary', 'Recevoir le lien');
    send.onclick = async () => {
      const email = ip.value.trim();
      if (!email) { ip.focus(); return; }
      send.disabled = true; send.textContent = 'Envoi…';
      try { await sync.connect(email); toast('Lien envoyé — vérifiez votre email'); close(); }
      catch (e) { console.warn(e); toast('Échec de l’envoi'); send.disabled = false; send.textContent = 'Recevoir le lien'; }
    };
    foot.appendChild(reconf); foot.appendChild(send);
    m.appendChild(foot);
  }
  ...
  function renderConnected(s, body, rerender, close, toast) {
    body.appendChild(el('p', null,
      '<span style="font-size:13px">Connecté : <strong>' + s.user.email + '</strong>' +
      '<br><span style="color:var(--cv-fg-muted);font-size:12px">Backend : ' + s.provider +
      (s.pending ? ' · envoi en cours…' : ' · à jour') + '</span></span>'));

    const row = el('div', 'field');
    const syncBtn = el('button', 'btn', '<i class="ph ph-arrows-clockwise"></i> Synchroniser maintenant');
    syncBtn.onclick = async () => {
      syncBtn.disabled = true;
      try { await sync.syncNow(); toast('Synchronisé'); } catch (e) { toast('Échec de la sync'); }
    ...
    forget.onclick = async () => { await sync.forget(); render(); };
    const out = el('button', 'btn', 'Se déconnecter');
    out.onclick = async () => { await sync.signOut(); render(); };
    const done = el('button', 'btn primary', 'Fermer'); done.onclick = close;
    foot.appendChild(forget); foot.appendChild(out); foot.appendChild(done);
    m.appendChild(foot);
  }
